/********************************************************************
	@ Name:		CLHelper
	@ Description:	A set of APIs to make programming OpenCL easier
	@ Developer:	Jianbin Fang from TU Delft
	@ Date:		27/12/2010
	@ Revision:	20/11/2013
********************************************************************/
...
/*------------------------------------------------------------------
	TODO List:
	(1) exception report is still a big issue.
	(2)
-------------------------------------------------------------------*/
...
/*------------------------------------------------------------
	...
	@return:
	@date:		24/03/2011
	@bug: when the argv[]='operand', i.e., the second char is 'p' or 't' or 'd', 
		the whole output might be wrong. 
	@revised date: 06/12/2013
------------------------------------------------------------*/
int platform_id = 0;
char device_type[3];
int device_id = 0;
...
/*------------------------------------------------------------
	...
	@revised 2: report the program building information
	@revised date: 20/11/2013 by Jianbin
------------------------------------------------------------*/
void _clInit(int platform_id, string device_type, int device_id = 0)throw(string){
   ...
   //--cambine-4: Create an OpenCL command queue    
	...
	/**
		bug report: 20/11/2013 by Jianbin
		when building a program for multiple devices, the problem will \
		exit encountering a bug for device 0.
		bug solution: build the problem in a device-by-device manner
		That is, replacing deviceListSize with 1, and using &(oclHandles.devices[DEVICE_ID_INUSED])
	**/
	...
	std::cout<<"Kernel file: "<<kernel_entry[i].file_name<< std::endl;
	...
	std::string options= "";	
	resultCL = clBuildProgram(oclHandles.program, 1, &(oclHandles.devices[DEVICE_ID_INUSED]), options.c_str(), NULL,  NULL);
	...
	if ((resultCL != CL_SUCCESS) || (oclHandles.program == NULL)){
		...
		size_t length;
        	resultCL = clGetProgramBuildInfo(oclHandles.program, 
        	                                ...
        	                                &length);
		if(resultCL != CL_SUCCESS) 
        	    cerr << "InitCL()::Error: Getting Program build info(clGetProgramBuildInfo)" << endl;

		char* buffer = (char*)malloc(length+1);
		if(buffer==NULL) {printf("Create buffer failed!\n"); exit(-1);}
		resultCL = clGetProgramBuildInfo(oclHandles.program, 
        	                                oclHandles.devices[DEVICE_ID_INUSED], 
        	                                CL_PROGRAM_BUILD_LOG, 
        	                                length, 
		...
		if(fp==NULL) {printf("Create file failed!\n"); exit(-1);}
		fprintf(fp, "%s\n", buffer);
		fclose(fp);
    	...
    	{
        	cerr << "Build Program Successfully!!" << endl;	
		//size_t length;
        	//resultCL = clGetProgramBuildInfo(oclHandles.program, \
        	                                oclHandles.devices[DEVICE_ID_INUSED], \
        	                                CL_PROGRAM_BUILD_LOG, \
        ...
        std::cout<<"Kernel name: "<<kernel_entry[i].function_name<<std::endl;
    ...
    std::cout<<"We have built "<<oclHandles.kernel.size()<<" kernel(s) in total."<<std::endl;
