...
kernel='kernels.cl'
kernelT='./template/kernels4.cl'
report='lmReport.txt'
# ------------------------------------------------------------------
# 			start execution 
# ------------------------------------------------------------------
for index in {16..24..1}; do # for each map (ignore MAP-401 [MAP-15] for now)
	...
	for (( n=${N}; n<${N}+1; n++ ))
		...
		for (( m=0; m<${n}+1; m++ ))	
		  do
			...
			inData="${idxx}for(int rr=-R; rr<=R; rr++){\n\t\t\t\tfor(int cc=-R; cc<=R; cc++){\n\t\t\t\t\t"
			inData="${inData}int hh=idxH+rr;\n\t\t\t\t\t"
			inData="${inData}if(hh<0) hh=0;\n\t\t\t\t\t"
			inData="${inData}if(hh>=rdimIn) hh=(rdimIn-1);\n\t\t\t\t\t"
			inData="${inData}int ww=idxW+cc;\n\t\t\t\t\t"
			inData="${inData}if(ww<0) ww=0;\n\t\t\t\t\t"
			inData="${inData}if(ww>=cdimIn) ww=(cdimIn-1);\n\t\t\t\t\t"
			inData="${inData}int idx=(hh)*cdimIn+(ww);\n\t\t\t\t\tval+="
			for (( nn=0; nn<${n}; nn++ ))
			  do
			    inData="${inData}in${nn}[idx]+"
			  done
			inData="${inData%?};\n\t\t\t\t}"
			inData="${inData}\n\t\t\t}"
			...
			if [ "${bits:2:2}" == "11" ]; then # get the last two bits of eMAP
				ceD2="\tint dl = (tly+R+WG) * WD + (tlx+R);\n\t\tint dg = (bx+tlx)+(by+tly+WG)*cdimIn;"
				for (( mm=0; mm<${m}; mm++ ))
				  do
				    ceD2="${ceD2}\n\t\tLM${mm}[dl] = in${mm}[dg];"
				  done
			...
			if [ "${bits:0:2}" == "11" ]; then # get the last two bits of eMAP
				ceD3="\tint dl = (tly+R) * WD + (tlx+R+WG);\n\t\tint dg = (bx+tlx+WG)+(by+tly)*cdimIn;"
				for (( mm=0; mm<${m}; mm++ ))
				  do
				    ceD3="${ceD3}\n\t\tLM${mm}[dl] = in${mm}[dg];"
				  done
			...
			if [ "${bits:0:2}" == "11" ] && [ "${bits:2:2}" == "11" ]; then # get the last two bits of eMAP
				ceD4="\tint dl = (tly+R+WG) * WD + (tlx+R+WG);\n\t\tint dg = (bx+tlx+WG)+(by+tly+WG)*cdimIn;"
				for (( mm=0; mm<${m}; mm++ ))
				  do
				    ceD4="${ceD4}\n\t\tLM${mm}[dl] = in${mm}[dg];"
				  done
			...
			lrD="${lrD}\t\tif(bbx>=cdimIn) bbx = (cdimIn - 1);\n"
			lrD="${lrD}\t\tdl = (tly+R) * WD + (tlx+R+${offsetH});\n"
			lrD="${lrD}\t\tdg = (bbx) + (bby) * cdimIn;\n"
			...
			coD="${coD}\t\tif(bbx>=cdimIn) bbx = (cdimIn-1);\n"
			coD="${coD}\t\tbby = by + tly - R;\n"
			coD="${coD}\t\tif(bby<0) bby = 0;\n"
			coD="${coD}\t\tdl = (tly) * WD + (tlx+R+${offsetH});\n"
			coD="${coD}\t\tdg = (bbx) + (bby) * cdimIn;\n"
			for (( mm=0; mm<${m}; mm++ ))
			  do
			    coD="${coD}\t\tLM${mm}[dl] = in${mm}[dg];\n"
			  done
			...
			lmUse="int cc = 0;\n"
			lmUse="${lmUse}\tint rr = 0;\n"
			lmUse="${lmUse}\tint idxww = (${idxww}+R);\n"
			lmUse="${lmUse}\tint idxhh = (${idxhh}+R);\n"
			lmUse="${lmUse}\tfor(rr=-R; rr<=R; rr++){\n"
			lmUse="${lmUse}\t\tfor(cc=-R; cc<=R; cc++){\n"
			lmUse="${lmUse}\t\t\tint idx=(idxhh+rr)*WD+(idxww+cc);\n"
			for (( mm=0; mm<${m}; mm++ ))
			  do
			    lmUse="${lmUse}\t\t\tretVal${mm} += LM${mm}[idx];\n"
			  done
			...
			gmUse="${gmUse}\t\t\tint hh=idxH+rr;\n"
			gmUse="${gmUse}\t\t\tif(hh<0) hh=0;\n"
			gmUse="${gmUse}\t\t\tif(hh>=rdimIn) hh=(rdimIn-1);\n"
			gmUse="${gmUse}\t\t\tint ww=idxW+cc;\n"
			gmUse="${gmUse}\t\t\tif(ww<0) ww=0;\n"
			gmUse="${gmUse}\t\t\tif(ww>=cdimIn) ww=(cdimIn-1);\n"
			gmUse="${gmUse}\t\t\tint idx=(hh)*cdimIn+(ww);\n"
			for (( mm=${m}; mm<${n}; mm++ ))
			  do
			    gmUse="${gmUse}\t\t\tretVal${mm}+=in${mm}[idx];\n"
			  done
			gmUse="${gmUse}\t\t}\n"
			gmUse="${gmUse}\t}\n"
