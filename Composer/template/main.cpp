...
void OMPRun(@argOMP, float * out, int cdim, int rdim, @argcdimIn, @argrdimIn);
void OCLRun(@argOCL, cl_mem out, int cdim, int rdim, @argcdimIn, @argrdimIn);
...
try{
	...
	OCLRun(@oclArgs, dOut, cdim, rdim, @oclcdimIn, @oclrdimIn);

#ifdef VARIFY	
	//OMPRun(hIn1, hIn2, rOut, cdim, rdim);
	OMPRun(@ompArgs, rOut, cdim, rdim, @oclcdimIn, @oclrdimIn);
#endif //VARIFY
