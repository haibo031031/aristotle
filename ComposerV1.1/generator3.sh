...
# 			start execution 
# ------------------------------------------------------------------
for index in {12..14..1}; do # for each map
		...
		for (( m=0; m<${n}+1; m++ ))	
		  do
			...
			if [ "${bits:2:2}" == "11" ]; then # get the last two bits of eMAP
				ceD2="\t\tint dl = (tly+R+WG) * WD + (tlx+R);\n\t\t\tint dg = (bx+tlx)+(by+tly+WG+rr)*cdimIn;"
				for (( mm=0; mm<${m}; mm++ ))
				  do
				    ceD2="${ceD2}\n\t\t\tLM${mm}[dl] = in${mm}[dg];"
				  done
			...
			if [ "${bits:0:2}" == "11" ] && [ "${bits:2:2}" == "11" ]; then # get the last two bits of eMAP
				ceD4="\t\tint dl = (tly+R+WG) * WD + (tlx+R+WG);\n\t\t\tint dg = (bx+tlx+WG)+(by+tly+WG+rr)*cdimIn;"
				for (( mm=0; mm<${m}; mm++ ))
				  do
				    ceD4="${ceD4}\n\t\t\tLM${mm}[dl] = in${mm}[dg];"
				  done
			...
			for (( mm=${m}; mm<${n}; mm++ ))
			  do
				# do some work here
				gmUse="${gmUse}retVal${mm}+=in${mm}[gmIdx+rr*cdimIn];\n\t\t"
			  done
			gmUse="${gmUse%?}"
			gmUse="${gmUse%?}"
