...
for index in {0..8..1}; do # for each map
	...
	for (( n=4; n<${N}+1; n++ ))
		...
		for (( m=0; m<${n}+1; m++ ))	
		  do
			...
			for (( mm=0; mm<${m}; mm++ ))
			  do
			    lmUse="${lmUse}retVal${mm} += LM${mm}[useIdx];\n\t"
			  done
			lmUse="int useIdx=(${idxH})*WD+(${idxW});\n\t${lmUse}"
			lmUse="${lmUse%?}"
			lmUse="${lmUse%?}"
			cat ${dirName}/${kernel} | sed "s/@lmUse/${lmUse}/g" > ./tmp
			cat ./tmp > ${dirName}/${kernel}
			# TODO: global load (n-m)
			idxW="" # x coodinate
			case "${bits:0:2}" in
				"00")  
				    idxW="0"
			...
			for (( mm=${m}; mm<${n}; mm++ ))
			  do
				# do some work here
				gmUse="${gmUse}retVal${mm}+=in${mm}[gmIdx];\n\t"
			  done
			gmUse="${gmUse%?}"
			gmUse="${gmUse%?}"
